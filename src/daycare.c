...
static void TransferEggMoves(void)
{
    ...
    for (i = 0; i < DAYCARE_MON_COUNT; i++)
    {
        ...
        eggSpecies = GetEggSpecies(GetMonData(mon, MON_DATA_SPECIES, NULL));
        numEggMoves = GetEggMoves(eggSpecies, sHatchedEggEggMoves);
...
static void BuildEggMoveset(struct Pokemon *egg, struct BoxPokemon *father, struct BoxPokemon *mother)
{
    ...
    eggSpecies = GetEggSpecies(GetMonData(egg, MON_DATA_SPECIES, NULL));
    numEggMoves = GetEggMoves(eggSpecies, sHatchedEggEggMoves);
