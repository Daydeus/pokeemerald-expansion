...
static void PrintMonWeight(u16 weight, u8 left, u8 top)
{
    ...
    {
        bufferMetric[i++] = EXT_CTRL_CODE_BEGIN;
        bufferMetric[i++] = EXT_CTRL_CODE_CLEAR_TO;
        i++;
        bufferMetric[i++] = CHAR_SPACE;
        bufferMetric[i++] = CHAR_SPACE;
        bufferMetric[i++] = CHAR_SPACE;
        bufferMetric[i++] = CHAR_SPACE;
        bufferMetric[i++] = CHAR_SPACE;
        ...
        bufferMetric[i++] = (((weight % 1000) % 100) / 10) + CHAR_0;
        bufferMetric[i++] = CHAR_COMMA;
        bufferMetric[i++] = (((weight % 1000) % 100) % 10) + CHAR_0;
        bufferMetric[i++] = CHAR_SPACE;
        bufferMetric[i++] = CHAR_k;
        bufferMetric[i++] = CHAR_g;

        bufferMetric[i++] = EOS;
        bufferMetric[2] = offset;
        PrintInfoScreenText(bufferMetric, left, top);
    }
}
